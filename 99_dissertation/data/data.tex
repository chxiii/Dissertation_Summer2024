...
More noteworthy is the relationship between the independent and dependent variables in Figure 3.4. The dependent variable \texttt{popgap} is significantly related only to \texttt{potato\_price}, \texttt{ground\_rent}, \texttt{grain\_acre\_total} and \texttt{imports\_total}. Also there is no high correlation between the independent variables. However, non-significant correlation coefficients do not directly lead to the conclusion that they should be excluded from the regression model \textemdash\ this tends to lead us to ignore relationships that are not linear in nature. This paper will further explore these potential nonlinear relationships in the next chapter, along with a discussion of the regression methods used.
